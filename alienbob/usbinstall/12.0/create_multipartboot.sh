...
cat <<"EOT"
# ------------------------------------------------------------------------#
# $Id: create_multipartboot.sh,v 1.20 2008/02/01 13:40:12 root Exp root $ #
# ------------------------------------------------------------------------#

EOT

IMGSIZE=253         # default disk image size in MB
FATSIZE=14000       # size of the FAT partition in kB
                    # Don't try to make this less than 8100
...
SLACKVERSION=${SLACKVERSION:-"12.0"}
...
#RESCANSCSIBUS="${SLACKROOT}/slackware/a/sysvinit-scripts-*.tgz"
...
# Space separated:
EXCL_256="ap/hplip ap/linuxdoc-tools n/samba n/net-snmp"
EXCL_512="d/clisp d/gcc-java d/gcc-gfortran d/gcc-gnat l/jre l/lesstiff n/net-snmp"
EXCL_1024="d/clisp d/gcc-java d/gcc-gfortran d/gcc-gnat kde/koffice xap/seamonkey"
EXCL_2048=""
EXCL_4096=""
...
if [ $OPTION -lt $IMGS_256 ] ; then
  ...
  echo "--- Targeting a 256 MB USB drive: ---"
  ADDPKGS=${PKGS_256}
  REMPKGS=${EXCL_256}
elif [ $OPTION -lt $IMGS_1024 ] ; then
  echo "--- Targeting a 512 MB USB drive: ---"
  ADDPKGS="${PKGS_512}"
  REMPKGS="${EXCL_512}"
elif [ $OPTION -lt $IMGS_2048 ] ; then
  echo "--- Targeting a 1024 MB USB drive: ---"
  ADDPKGS="${PKGS_1024}"
  REMPKGS="${EXCL_1024}"
elif [ $OPTION -lt $IMGS_4096 ] ; then
  echo "--- Targeting a 2048 MB USB drive: ---"
  ADDPKGS="${PKGS_2048}"
  REMPKGS="${EXCL_2048}"
  EXTRAARGS="--exclude bootdisks \
...
echo "--- Partitioning image with  FAT type 4 (FAT16) and Linux partitions: ---"
...
# Check if we have a newer initrd:
if [ -f $SRCDIR/$INITRDIMG ]; then
...
#installpkg -root ${STAGING}/work/ $RESCANSCSIBUS 1>/dev/null
#cp ${STAGING}/work/sbin/rescan-scsi-bus ${STAGING}/newinitrd/sbin/
#patch -p0 ${STAGING}/newinitrd/sbin/rescan-scsi-bus $SRCDIR/rescan-scsi-bus.diff
...
#patch -p0 ${STAGING}/newinitrd/usr/lib/setup/SeTkernel $SRCDIR/SeTkernel.diff
patch -p0 ${STAGING}/newinitrd/etc/rc.d/rc.usb $SRCDIR/rc.usb.diff
patch -p0 ${STAGING}/newinitrd/bin/network $SRCDIR/network.diff
patch -p0 ${STAGING}/newinitrd/bin/pcmcia $SRCDIR/pcmcia.diff
